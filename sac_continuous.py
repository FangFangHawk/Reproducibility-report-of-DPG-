...
if __name__ == "__main__":
    ...
    wandb.init(project="A2C", config=config, name="n=1_continuous")
    args = wandb.config
    ...
    for i_episode in range(args.num_episode):
        ...
        while not done:
            global_step += 1
            step += 1
            paras = agent.actor(obs)
            dist = Normal(loc=paras, scale=0.1)
            action = dist.sample()
            log_pi = dist.log_prob(action)
            action = [np.clip(action.item(), -1, 1)*2]
